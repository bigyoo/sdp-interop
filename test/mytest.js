...
QUnit.test('01-planb-offer-from-jvb-conversion-to-unified-plan', function(assert) {
	...
	const planb = fs.readFileSync(BASE_PATH_OF_SAFARI_SDP_FOLDER + '01-offer-from-jvb.txt', 'utf8');
	...
	assertAnswerAfterEveryTransformationApplied(assert, answerRTCSEssionDesc.sdp);
...
var assertAnswerAfterEveryTransformationApplied = function(assert, actualSdp) {
	console.log('> Checking answer after ever Transformation applied');
	
	// read expected result
	const expected = fs.readFileSync(BASE_PATH_OF_SAFARI_SDP_FOLDER + '09-answer-after-every-transformation-applied.txt', 'utf8');
	assert.equal(expected.trim(), actualSdp.trim());
}
